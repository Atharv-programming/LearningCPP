...
class MarksOperation
{
    public :
        ...
        float AvgMarks(Subject P, Subject C, Subject M, Subject E, Subject B);
};   
...
int main(void)
{
    ...
    std::cout<<"Percentage : "<<MO.Percentage(Physics, Chemistry, Maths, English, Biology)<<"%"<<std::endl;
    std::cout<<"Avg Marks : "<<MO.AvgMarks(Physics, Chemistry, Maths, English, Biology)<<std::endl;

    return(0);
}

float MarksOperation::Percentage(Subject P, Subject C, Subject M, Subject E, Subject B)
{
    return(((P.marks + C.marks + M.marks + E.marks+ B.marks)/500.0f) * 100.0f);
}

float MarksOperation::AvgMarks(Subject P, Subject C, Subject M, Subject E, Subject B)
{
    return((P.marks + C.marks + M.marks + E.marks+ B.marks)/5);
}
